package Arrays;

import ST.st;

public class TrappingRainwater22 {

    // (OWN) -> optimised for space complexity
    public static int trap(int[] height) {
        int n = height.length;
        int max = 0;

        // index of highest elevation;
        for (int i = 1; i < n; i++) {
            if (height[i] > height[max]) {
                max = i;
            }
        }

        int trappedWater = 0;
        int leftMax = 0;
        // from left to highest elevation;
        for (int i = 1; i < max; i++) {
            if (height[i] > height[leftMax]) {
                leftMax = i;
            }
            trappedWater += height[leftMax] - height[i];
        }

        int rightMax = n - 1;
        // from right to highest elevation;
        for (int i = n - 2; i > max; i--) {
            if (height[i] > height[rightMax]) {
                rightMax = i;
            }
            trappedWater += height[rightMax] - height[i];
        }

        return trappedWater;
    }

    // (DIDI)
    public static int trappedRainwater(int height[]) {
        int n = height.length;
        // calculate left max boundary - array
        int leftMax[] = new int[n];
        leftMax[0] = height[0];
        for(int i=1; i<n; i++) {
            leftMax[i] = Math.max(height[i], leftMax[i-1]);
        }

        // calculate right max boundary - array
        int rightMax[] = new int[n];
        rightMax[n-1] = height[n-1];
        for(int i=n-2; i>=0; i--) {
            rightMax[i] = Math.max(height[i], rightMax[i+1]);
;        }

        int trappedWater = 0;
        // loop
        for(int i=0; i<n; i++) {
            // waterLevel = min(left[i], right[i])
            int waterLevel = Math.min(leftMax[i], rightMax[i]);

            // trapped water = waterLevel - height[i];
            trappedWater += waterLevel - height[i];
        }

        return trappedWater;
    }

    public static void main(String[] args) {
        int height[] = {4, 2, 0, 6, 2, 3, 5};
        int trappedWater =  trap(height);
        // trappedWater = trappedRainwater(height);
        System.out.println(trappedWater);
    }
}
